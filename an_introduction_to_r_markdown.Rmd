...
- v1: 基於package markdown之版本
- v2: 基於[pandoc](http://johnmacfarlane.net/pandoc/)之R markdown，整合於RStudio v0.98.932之後的版本
    - default bootstrap template
    - docx and pdf
    - embedded shiny apps
    - slides with ioslides and Beamer
- 若要繼續使用R Markdown v1，可以在文件最一開始加上特殊的註解
...
- YAML Metadata: 設定文件之基本屬性，格式為[YAML](http://en.wikipedia.org/wiki/YAML)
- Markdown: 標題與報告內容
- R Code Chunk: 可執行之R程式碼 
...
- `dev`('png'): 圖片輸出的格式(png, jpeg, tiff, svg)
- `fig.path`('figure/'): 圖片輸出之路徑
- `fig.width`(7): 圖片之寬度
- `fig.height`(7): 圖片之高度
...
Note that content will flow across the columns.
